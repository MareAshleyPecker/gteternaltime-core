...
|------------------------------|-------------------------|-------|-----------------------------------------------|--------------------------------------------------------------------------|
| `gtmthings-1.6.0/`           | GTMThings               | 1.6.0 | liansishen/GTMThings（LGPLv3.0，原作者 liansishen） | 高级终端：界面改成 GTO 样式 + 对应文案（`ui/`+`lang/`）；再把 GTO 的**模块搭建 / 镜像搭建 / 拆除模式**三项搭建行为与重复次数上限补进逻辑（`logic/`，见该目录 `NOTES.md` §6）；第三轮把窗口从 300×172 放大到 **372×274**（右侧每块可见 7 行，见 §2/§3）并把右侧两块面板改成「不扫描也能列出 5 类可选部件」（见 §9，数据由 GTET 侧预置） |
...
| 补丁         | 外部源码检出（写死）                                  | 构建命令                                        | 产物落点                                                                                |
|------------|---------------------------------------------|---------------------------------------------|-------------------------------------------------------------------------------------|
| GTM 7.5.3  | `D:\java\GregTech-Modern-7.5.3-1.20.1`      | `gradlew build publishToMavenLocal`         | `%USERPROFILE%\.m2\repository\com\gregtechceu\gtceu\gtceu-1.20.1\7.5.3\gtceu-1.20.1-7.5.3.jar` |
| GTMThings  | `D:\java\GTMThings-1.6.0`                   | `gradlew spotlessApply build` **→ 自动补资源 → 体检** | 拷成 `libs/jarjar/gtmthings-1.6.0-forge.jar`                                          |
...
2. 跑 `scripts/gtmt-resfix.ps1`：把仓内 vendored 的
...
4. **体检通过才**覆盖 `libs/jarjar/gtmthings-1.6.0-forge.jar`；不通过就抛 `GradleException`
   让任务失败，并明确说「libs/jarjar 没有被覆盖」。

只做 2~4 步（不跑外部构建）的独立任务：

```
gradlew resfixGtmtJar                                    # 输入=外部检出 build/libs，输出=libs/jarjar
...
compile/runtime 依赖直接指向 `libs/jarjar/gtmthings-1.6.0-forge.jar`（`fg.deobf("jarjar:gtmthings-1.6.0:forge")`，
flatDir 仓库 `dir "libs/jarjar"`）—— 以前 dev 用的是 curse 上的官方原件，所以补丁**在 dev 里根本没生效**。
...
- GTMThings（**2026-09-13 新增**）：`assets/**` 条目数 ≥ 1080，且存在以 `assets/gtmthings/models/block/`
  ...
  `libs/jarjar`），所以「缺资源的补丁 jar」现在既产不出也进不了包。

## 资源补齐链路的三块拼图（2026-09-13 固化）

| 文件 | 作用 |
|---|---|
| `scripts/gtmt-resfix.ps1` | 参数化（输入 jar / 参考 jar / 输出 jar）的资源补齐脚本；三条硬约束：已存在条目一律跳过、拒绝任何 `.class`、跳过 `.cache/`；缺我们改过的 lang 就直接报错（绊线） |
...
⚠️ 脚本里找 PowerShell 用的是**绝对路径** `%SystemRoot%\System32\WindowsPowerShell\v1.0\powershell.exe`：
这台机器上 `powershell.exe` 不在 PATH 里（`Get-Command powershell.exe` 找不到），写 `powershell` 会直接
`Task ... FAILED`。脚本的 stdout 一律 ASCII（中文经 Gradle 的 Exec 捕获会因代码页不匹配变乱码），
文件本身带 UTF-8 BOM 以便 Windows PowerShell 5.1 正确读中文注释。

## ⚠️ 换了 `libs/jarjar` 里 jar 的**内容**但 flatDir 坐标没变 → 必须手工清 deobf 缓存
...
```
<repo>\gradlew.bat <tasks> --console=plain --no-daemon ^
  "-Dorg.gradle.java.installations.paths=D:\Java\zulu-17\zulu17.68.203-ca-jdk17.0.20.1-win_x64" ^
  "-Dorg.gradle.jvmargs=-Xmx512m" "-Dorg.gradle.workers.max=1"
```

- 两个检出都要求 **JDK 17**（toolchain 写死 17）；`JAVA_HOME` 指上面那个 zulu-17。
- `--no-daemon` + 单 worker + 小堆是**必须**的：NeoForm 反编译峰值约 600MB，
  24G 内存/页面文件打满的机器上给多了会 `os::commit_memory(...) failed ... DOS error 1455`。
- ⚠️ GTM 检出自己的 `gradle.properties` 里有一行失效的
  `org.gradle.java.home=D:/.gradle/jdks/eclipse_adoptium-17-amd64-windows/jdk-17.0.19+10`（该路径不存在）。
  `scripts/patches.gradle` 用命令行 `-Dorg.gradle.java.home=<zulu-17>` 覆盖它；
  手工跑外部构建时不加这一项会直接 `Java home supplied in org.gradle.java.home is invalid`。

## ⚠️ 注意：`gtceu-7.5.3/logic/` 是**归档**，别整目录覆盖到检出

GTM 检出 `D:\java\GregTech-Modern-7.5.3-1.20.1` 里那 8 个文件本来就是**补丁版**
...
`D:\java\GTMThings-1.6.0\gradle.properties` 的 `gtceu_version` 还是上游默认的 `7.5.2`，
...
② 构建时覆盖 `-Pgtceu_version=7.5.3` 让它走 mavenLocal 里那份补丁构建（未实测）。
...
3. GTMThings 手工把产物拷到 `libs/jarjar/gtmthings-1.6.0-forge.jar`；
   GTM 手工把产物拷到 mavenLocal 那个坐标（自动流程用的是 `publishToMavenLocal`）。
